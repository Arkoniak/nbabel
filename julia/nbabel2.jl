...
Force calculation.
"""
function compute_acceleration2(pos, mass, acc)
    N = length(pos)
...
function compute_acceleration(pos, mass, acc)
    ...
    @inbounds for i in 1:N
        ...
        @fastmath @inbounds @simd ivdep for j = i+1:N
            dr = pos_i .- pos[j]
...
function read_ICs(fname)
    ...
    pos = Vector{Tuple{Float64, Float64, Float64}}(undef, N)
    vel = Vector{Tuple{Float64, Float64, Float64}}(undef, N)

    id = @view ICs[:, 1]
    mass = @view ICs[:, 2]

    for i in axes(ICs, 1)
        pos[i] = (ICs[i, 3], ICs[i, 4], ICs[i, 5])
    end

    for i in axes(ICs, 1)
        vel[i] = (ICs[i, 6], ICs[i, 7], ICs[i, 8])
    end
